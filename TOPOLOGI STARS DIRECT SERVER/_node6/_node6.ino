...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("ESP8266Client6", mqtt_user, mqtt_password)) {
...
void printValues() {
    ...
    if(client.publish("Humidity-node6", String(dht.readHumidity()).c_str() )){
    ...
    digitalWrite(redLed, LOW);
      ...
      Serial.println();
      client.connect("ESP8266Client6", mqtt_user, mqtt_password);
      client.publish("Humidity-node6", String(dht.readHumidity()).c_str() );
    }
    
    if(client.publish("Humidity-node6", String(bme.readHumidity()).c_str() )){
      Serial.print("Humidity telah dikirm");
      digitalWrite(greenLed, HIGH);
      digitalWrite(redLed, LOW);
    } 
      ...
      Serial.println();
      client.connect("ESP8266Client6", mqtt_user, mqtt_password);
      client.publish("Humidity-node6", String(bme.readHumidity()).c_str() );
    }

    //TEMPERATURE CLIENT PUBLISH
    if(client.publish("Temperature-node6", String(dht.readTemperature()).c_str() )){
      ...
      Serial.println();
      client.connect("ESP8266Client6", mqtt_user, mqtt_password);
      client.publish("Temperature-node6", String(dht.readTemperature()).c_str() );
    }

    if(client.publish("Temperature-node6", String(bme.readTemperature()).c_str() )){
      Serial.print("Temperature telah dikirm");
      digitalWrite(greenLed, HIGH);
      digitalWrite(redLed, LOW);
    } 
      ...
      Serial.println();
      client.connect("ESP8266Client6", mqtt_user, mqtt_password);
      client.publish("Temperature-node6", String(bme.readTemperature()).c_str() );
    }

    //MOISTURE CLIENT PUBLISH
    if(client.publish("Moisture-node6", String(soilMoisture).c_str())){
      ...
      Serial.println();
      client.connect("ESP8266Client6", mqtt_user, mqtt_password);
      client.publish("Moisture-node6", String(soilMoisture).c_str());
    }

    //RUIN CLIENT PUBLISH
    if(client.publish("Rain-node6", String(rain).c_str())){
      ...
      Serial.println();
      client.connect("ESP8266Client6", mqtt_user, mqtt_password);
      client.publish("Rain-node6", String(rain).c_str());
    }

    //PRESSURE AIR CLIENT PUBLISH
    if(client.publish("Pressure-node6", String(bme.readPressure() / 100.0F).c_str())){
      ...
      Serial.println();
      client.connect("ESP8266Client6", mqtt_user, mqtt_password);
      client.publish("Pressure-node6", String(bme.readPressure() / 100.0F).c_str());
    }
    
    //APPROX ALTITUDE CLIENT PUBLISH
    if(client.publish("Altitude-node6", String(bme.readAltitude(SEALEVELPRESSURE_HPA)).c_str())){
      ...
      Serial.println();
      client.connect("ESP8266Client6", mqtt_user, mqtt_password);
      client.publish("Altitude-node6", String(bme.readAltitude(SEALEVELPRESSURE_HPA)).c_str());
    }
