...
void printValues() {
    ...
    if(client.publish("Humidity-node1", String(dht.readHumidity()).c_str() )){
    ...
    digitalWrite(redLed, LOW);
      ...
      client.publish("Humidity-node1", String(dht.readHumidity()).c_str() );
    }
    
    if(client.publish("Humidity-node1", String(bme.readHumidity()).c_str() )){
      Serial.print("Humidity telah dikirm");
      digitalWrite(greenLed, HIGH);
      digitalWrite(redLed, LOW);
    } 
      ...
      Serial.println();
      client.connect("ESP8266Client1", mqtt_user, mqtt_password);
      client.publish("Humidity-node1", String(bme.readHumidity()).c_str() );
    }

    //TEMPERATURE CLIENT PUBLISH
    if(client.publish("Temperature-node1", String(dht.readTemperature()).c_str() )){
      ...
      Serial.println();
      client.connect("ESP8266Client1", mqtt_user, mqtt_password);
      client.publish("Temperature-node1", String(dht.readTemperature()).c_str() );
    }

    if(client.publish("Temperature-node1", String(bme.readTemperature()).c_str() )){
      Serial.print("Temperature telah dikirm");
      digitalWrite(greenLed, HIGH);
      digitalWrite(redLed, LOW);
    } 
      ...
      Serial.println();
      client.connect("ESP8266Client1", mqtt_user, mqtt_password);
      client.publish("Temperature-node1", String(bme.readTemperature()).c_str() );
    }

    //MOISTURE CLIENT PUBLISH
    if(client.publish("Moisture-node1", String(soilMoisture).c_str())){
      ...
      Serial.println();
      client.connect("ESP8266Client1", mqtt_user, mqtt_password);
      client.publish("Moisture-node1", String(soilMoisture).c_str());
    }

    //RUIN CLIENT PUBLISH
    if(client.publish("Rain-node1", String(rain).c_str())){
      ...
      Serial.println();
      client.connect("ESP8266Client1", mqtt_user, mqtt_password);
      client.publish("Rain-node1", String(rain).c_str());
    }

    //PRESSURE AIR CLIENT PUBLISH
    if(client.publish("Pressure-node1", String(bme.readPressure() / 100.0F).c_str())){
      ...
      Serial.println();
      client.connect("ESP8266Client1", mqtt_user, mqtt_password);
      client.publish("Pressure-node1", String(bme.readPressure() / 100.0F).c_str());
    }
    
    //APPROX ALTITUDE CLIENT PUBLISH
    if(client.publish("Altitude-node1", String(bme.readAltitude(SEALEVELPRESSURE_HPA)).c_str())){
      ...
      Serial.println();
      client.connect("ESP8266Client1", mqtt_user, mqtt_password);
      client.publish("Altitude-node1", String(bme.readAltitude(SEALEVELPRESSURE_HPA)).c_str());
    }
